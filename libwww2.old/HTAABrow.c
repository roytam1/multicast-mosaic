...
This module is the browser side interface to Access Authorization (AA) package.
It contains code only for browser.
...
thinks that the given document is protected. Otherwise this function returns NULL.This function also calls the functions HTPrompt(),HTPromptPassword()
and HTConfirm() to get the username, password and some confirmation from the user.                                
...
typedef struct {
	...
	HT_BOOL	retry;		/* Failed last time -- reprompt (or whatever)*/
} HTAASetup;
...
void HTAASetup_clearall (HTList *s)
{
	HTList *n, *nn, *sn, *snn;
	HTAASetup *o;
...
*/
static HTAAServer *HTAAServer_new(const char *hostname, int portnumber)
{
	HTAAServer *server;
...
**			NULL, if not found.
*/
static HTAAServer *HTAAServer_lookup(const char *hostname, int portnumber)
{
	if (hostname) {
...
*/
static HTAASetup *HTAASetup_new (HTAAServer *	server,
				       char *	tmplate,
...
**	returns		nothing.
*/
static void HTAASetup_updateSpecifics (HTAASetup *setup,
...
*/
void HTAARealm_clearall (HTList *realm_table)
{
	HTList *n, *nn;
	HTAARealm *r;
...
**
*/
static char *compose_auth_string (HTAAScheme scheme,
				HTAASetup *setup,
				caddr_t appd)
{
...
/* marca added unsigned char * cast for HP. */
		...
		free(cleartext);
		free(ciphertext);
	} else if(scheme == HTAA_MD5) { /* the following added by DXP */
...
/* make md5_cleartext */
		*md5_cleartext = (unsigned char)0;
		...
		strcat((char*)md5_cleartext, nonce);
		strcat((char*)md5_cleartext, ":");
...
**			NO, otherwise.
*/
HT_BOOL HTAA_shouldRetryWithAuth (char * start_of_headers,
...
/* Read server reply header lines */
	...
	while (NULL != (line = HTAA_getUnfoldedLine(appd))  &&  *line != (char)0) {
		...
		if (strchr(line, ':')) {	/* Valid header line */
			...
			if (0==strcasecmp(fieldname, "WWW-Authenticate:")) {
				if (HTAA_UNKNOWN != (scheme = HTAAScheme_enum(arg1))) {
					HTList_addObject(valid_schemes, (void*)scheme);
					if (!scheme_specifics) {
						int i;

						scheme_specifics = (HTAssocList**) malloc(HTAA_MAX_SCHEMES * sizeof(HTAssocList*));
						if (!scheme_specifics)
			...
			} else if (0==strcasecmp(fieldname, "WWW-Protection-Template:")) {
				if (wWWParams.trace)
