...
static NewsArt *FirstArt = NULL;
static NewsArt *LastArt = NULL;
static int Count = 0;
static int GroupFirst = 0;
static int GroupLast = 0;
static int ReadLast = 0;
static int ReadFirst = 0;
static newsgroup_t *LastGroup = NULL; 
static NewsArt *NextArt = NULL;
...
static void AddArtTop (NewsArt *add)
{
    NewsArt *temp;
	...
	for(temp = LastArt; temp; temp = temp->prevt)
	    if( !strcmp(add->FirstRef,temp->FirstRef) 
		|| !strcmp(add->FirstRef,temp->ID) ) break;
    } else { 
	for(temp = LastArt; temp; temp = temp->prevt)
	    if(!strcmp(add->ID,temp->FirstRef)) break;
    }
...
static void start_anchor (const char * href, caddr_t appd)
{
  PUTS ("<A HREF=\"");
  PUTS (href);
  PUTS ("\">");
...
*/
static void write_anchor(const char *text, const char *addr,caddr_t appd)
{
...
*/
static void write_anchors (char *text, caddr_t appd)
{
    char * start = text;
    char * end;
    char c;
...
static void abort_socket (caddr_t appd)
{
...
**	4.	Default to "news"
*/
static HT_BOOL initialized = NO;
static HT_BOOL initialize (void)
{
  /*   Get name of Host  */
...
static int newswrite (const char *msg)
{
    int status;
...
static int response (const char *command, caddr_t appd)
{
  int result;    
...
static int OpenNNTP (caddr_t appd) 
{
    ...
    status = HTDoConnect (url, NEWS_PORT, &s,appd);

    if (status < 0) { /* HTNews: Unable to connect to news host */
	close(s);
	s = -1;
	return 2;
    } else {
...
NewsArt *is_news_url(char *ss)
{
    ...
    if (strchr (ss, '*'))
      return NULL;
    if((strlen(ss) > 5) && !strncmp("news:", ss, 5)){
	ss = &ss[5];
	/* check the obvious */
	if(CurrentArt && !strcmp(ss,CurrentArt->ID)) return CurrentArt;
	for(art = FirstArt; art; art = art->nextt){
	    if(!strcmp(ss,art->ID)) return art;
	    if(art->next)
		for(art2 = art->next; art2; art2 = art2->next)
		    if(!strcmp(ss,art2->ID)) return art2;
	}
    }
    return NULL;
}
...
/* Beginning in 2.7b4, these now return the next/prev unread article/thread ,
   ...
   is set in the news_next functions.  Note that because the previous state
   is saved in the news_next functions, they should only be called when 
   necessary! 
...
*/       
static void read_article (char * artID, caddr_t appd)
{
	int i;
		...
		if ((ch == LF) || (p == &line[LINE_LENGTH])) {            
			*--p=0;			/* Terminate the string */
			...
			if(line[0]<' ') {
				...
				case 'L':
				case 'l':
	    				if(!strcasecmp(line,"LINES:"))
	      					linecount = atoi(&line[6]);
	    				break;
	  			case 'F':
	  			case 'f':
	    				if(!strcasecmp(line, "FROM:")) 
	      					from = strdup(&line[5]);
	    				break;
	  			case 'O':
	  			case 'o':
	    				if(!strcasecmp(line, "ORGANIZATION:")) 
	      					org = strdup(&line[13]);
	    				break;
	  			case 'N':
	  			case 'n':
	    				if(!strcasecmp(line, "NEWSGROUPS:")) 
	      					newsgroups = strdup(&line[11]);
	    				break;
	  			case 'R':
	  			case 'r':
	    				if(!strcasecmp(line, "REFERENCES:")) 
	      					references = strdup(&line[11]);
	    				break;
	  			default: /* unknown headers ignored */
	    				break;
	  			}
			}
	    		...
	    		*p++=0;			/* Terminate the string */
	    		...
	    		if (line[0]=='.') {
            		...
            		case 3:
	      			...
	      			if(!strcasecmp(line,"CONTENT-TRANSFER-ENCODING: BASE64")){
					decode = 4;
            		...
            		case 4:
	      			...
	      			if(!line[0] || isspace(line[0])) {
					fp = startuudecode(filename = tempnam (mMosaicTmpDir,"mMo"));
					decode=2;
					p=line;
					continue;
	      			}
            		...
            		case 5: /* possible mime encap encoding crud */
	      			if(!strcasecmp(line,"CONTENT-TYPE: IMAGE")) {
					decode = 3;
					break;
	      			} else {   
					decode = 0;
	    		...
	    		/* HTTP, FTP, MAILTO, GOPHER,  */
					...
					*pp=0;
					...
					if((pp-m)>2) {
		  				if(!strcasecmp(m,"HTTP:") || !strcasecmp(m,"FTP:") ||
		     				!strcasecmp(m,"MAILTO:") || !strcasecmp(m,"NEWS:") ||
		     				!strcasecmp(m,"GOPHER")) {
		    					ll=*m;
		    					*m=0;
		    					PUTS(l);
...
*/ 
static void read_list (caddr_t appd)
{
...
static void XBuildArtList( const char *groupName, int first_required,
...
static void BuildArtList(const char *groupName, int first_required,
  int last_required, caddr_t appd)
{
    NewsArt *art;
...
/* thanks to Martin Hamilton for this bit 'o code...  his choice of header, not mine */
...
**
*/
static void read_group(char *groupName, int first, int last,
	caddr_t appd)
{
    NewsArt *art,*art2, *f;
    char *p;
...
/*		Load by name	 */
int HTLoadNews ( char *arg, HTParentAnchor *anAnchor,
	HTFormat format_out, caddr_t appd)
{
  ...
  HT_BOOL group_wanted;			/* Flag: group was asked for, not article */
  HT_BOOL list_wanted;			/* Flag: group was asked for, not article */
