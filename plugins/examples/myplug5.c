...
void MPP_New(MPX_INSTANCE *p)
{
  ...
  MOSAIC=(Widget)MPP_GetValue(p,MOSAICwin,NULL);
  my=MPP_GetMem(p,INSTANCE);
  
  printf("le file descriptor de mon fichier est : %d\n\n",monfichier);
  
  if((test = Traite_Donnee (monfichier))==0) printf("super ca marche\n");
  else printf("plouf\n\n");
...
void MPP_Initialize(MPX_PLUGIN myplug)
{
  ...
  string=(char*)MPP_MemAlloc(myplug,PLUG,sizeof("BONJOUR "));
  i=(int*)MPP_MemAlloc(myplug,PLUG,sizeof(int));
  *i=1;
