...
typedef enum { mo_new_annotation = 0, mo_edit_annotation } mo_amode;
...
/* Merge into pans */
	if (!pans && !grpans) {	 /* pans is already NULL, do nothing */
	} else if (pans && !grpans) {	 /* pans is all there is, do nothing */
	} else if (!pans && grpans) { 
	...
	if (pans == NULL) /* If we have no annotations, return NULL now */
		goto done;
...
static XmxCallback (annotate_win_cb1)
{
...
static XmxCallback (annotate_win_cb2)
{
	mo_window *win = (mo_window*)client_data;
	...
	mo_open_another_window (win,
		mo_assemble_help_url ("help-on-annotate-win.html"), NULL, NULL);
}

static XmxCallback (annotate_win_cb0)
{
	mo_window *win = (mo_window*)client_data;
	char *txt;
	int i;
	...
	txt = XmxTextGetString (win->annotate_text);
/*
...
/* This is a temporary fix because Eric's real fix doesn't seem to be working. */
 ...
 * if inside an HDF file, else the canonical form.  */
				mo_new_grpan(mo_url_to_unique_document(
					win->current_node->url), title, author, 
					txt);
...
/* Change the annotation on the server */
	...
	if (txt && *txt) {
		if (win->annotation_mode == mo_new_annotation) {
			mo_new_pan(mo_url_to_unique_document(
					win->current_node->url), 
...
static void do_slate (mo_window *win)
{
	...
	char namestr[1000]; /* passwd entries are now getting absurdly long. */
...
static XmxCallback (include_fsb_cb)
{
	...
	if (!fp) {
		...
		XmxMakeErrorDialog (win->annotate_win, final, 
...
XmxCallback (mo_annotation_private_cb)
{
	mo_window *win = (mo_window *)client_data;
...
XmxCallback (mo_annotation_workgroup_cb)
{
	mo_window *win = (mo_window *)client_data;
...
XmxCallback (mo_annotation_public_cb)
{
	mo_window *win = (mo_window *)client_data;
  
	win->pubpri = mo_annotation_public;
#ifdef GRPAN_PASSWD
	do_privacy (win);
#endif
}
...
/* Create it for the first time. */
	...
	win->annotate_win =XmxMakeFormDialog(win->base,"Mosaic: Annotate Window");
 ...
 * group annotations yet. */
	...
	win->pubpri_menu = XmxRMakeOptionMenu(annotate_form, "", pubpri_opts);
	XmxRSetSensitive(win->pubpri_menu, mo_annotation_public, XmxNotSensitive);
	if (!get_pref_string(eANNOTATION_SERVER))
		XmxRSetSensitive(win->pubpri_menu, 
			(XtPointer)mo_annotation_workgroup, XmxNotSensitive);
	win->pubpri = mo_annotation_private;
...
/* --- Frame for the password fields. --- */
	...
	win->passwd_toggle = XmxMakeToggleButton(passwd_f, 
		"Password Protection" , passwd_toggle_cb, win);
	...
	XmxSetConstraints (win->passwd_toggle, XmATTACH_FORM, XmATTACH_FORM, 
		XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
	XmxSetOffsets (win->passwd_label, 1, 0, 15, 10);
	XmxSetConstraints (win->passwd_label, XmATTACH_FORM, XmATTACH_FORM,
		XmATTACH_WIDGET, XmATTACH_NONE, NULL, NULL, 
		win->passwd_toggle, NULL);
	XmxSetOffsets (win->annotate_passwd, 1, 0, 6, 6);
	XmxSetConstraints (win->annotate_passwd, XmATTACH_FORM, XmATTACH_FORM,
		XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, 
		win->passwd_label, NULL);
#endif
	logo = XmxMakeNamedLabel (annotate_form, NULL, "logo");
	XmxApplyBitmapToLabelWidget(logo, (char*)annotate_bits,annotate_width,annotate_height);
	...
	buttons_form = XmxMakeFormAndThreeButtonsSqueezed(annotate_form, 
			"Commit" , "Dismiss" , "Help..." ,
			annotate_win_cb0, annotate_win_cb1, 
			annotate_win_cb2, (XtPointer) win);
  
/* Constraints for annotate_form. */
/* author_label: top form, bottom nothing, left form, right nothing. */
	XmxSetOffsets (author_label, 14, 10, 10, 10);
	XmxSetConstraints (author_label, XmATTACH_FORM, XmATTACH_NONE,
		XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
/* annotate_author: top form, bottom nothing, left author_label, right logo. */
	XmxSetOffsets (win->annotate_author, 10, 10, 10, 10);
	XmxSetConstraints (win->annotate_author, XmATTACH_FORM, XmATTACH_NONE,
		XmATTACH_WIDGET, XmATTACH_WIDGET,
		NULL, NULL, author_label, logo);
/* title_label: top annotate_author, bottom nothing, left form, right nothing */
	XmxSetOffsets (title_label, 14, 10, 10, 10);
	XmxSetConstraints (title_label, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_FORM, XmATTACH_NONE, 
		win->annotate_author, NULL, NULL, NULL);
/* annotate_title: top annotate_author, bottom nothing, left title_label, right logo. */
	XmxSetOffsets (win->annotate_title, 10, 10, 10, 10);
	XmxSetConstraints(win->annotate_title, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_WIDGET, XmATTACH_WIDGET,
...
#endif
	XmxSetConstraints (win->pubpri_menu->base, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_FORM, XmATTACH_NONE, win->annotate_title, NULL, NULL, NULL);
#ifdef GRPAN_PASSWD
/* passwd_frame: top annotate_title, bottom nothing, left pubpri_menu, right form */
	XmxSetOffsets (passwd_frame, 10, 10, 10, 10);
	XmxSetConstraints (passwd_frame, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_WIDGET, XmATTACH_FORM,
		win->annotate_title, NULL, win->pubpri_menu->base, NULL);
	XmxSetOffsets (sep, 10, 0, 0, 0);
	XmxSetConstraints (sep, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
...
/* text_label: top sep, bottom nothing, left form, right nothing */
	XmxSetOffsets (text_label, 12, 0, 10, 10);
	XmxSetConstraints (text_label, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_FORM, XmATTACH_NONE,
		sep, NULL, NULL, NULL);
  
/* Buttons */
	XmxSetOffsets (clear_button, 8, 0, 0, 3);
	XmxSetConstraints (clear_button, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_NONE, XmATTACH_WIDGET, sep, NULL, NULL, include_button);
	XmxSetOffsets (include_button, 8, 0, 0, 3);
	XmxSetConstraints (include_button, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_NONE, XmATTACH_WIDGET,
		sep, NULL, NULL, win->delete_button);
	XmxSetOffsets (win->delete_button, 8, 0, 10, 10);
	XmxSetConstraints (win->delete_button, XmATTACH_WIDGET, XmATTACH_NONE,
		XmATTACH_NONE, XmATTACH_FORM,
		sep, NULL, NULL, NULL);
  
/* logo: top form, bottom nothing, left nothing, right form. */
	XmxSetOffsets (logo, 5, 10, 10, 5);
	XmxSetConstraints(logo, XmATTACH_FORM, XmATTACH_NONE, 
		XmATTACH_NONE, XmATTACH_FORM,
		NULL, NULL, NULL, NULL);
	XmxSetOffsets (XtParent (win->annotate_text), 0, 2, 0, 0);
	XmxSetConstraints (XtParent (win->annotate_text), 
		XmATTACH_WIDGET, XmATTACH_WIDGET, 
...
mo_status mo_post_annotate_win(mo_window *win, int edit_flag, int id, 
		char *title, char *author, char *text, char *fname)
{
	if (!win->current_node)
		return mo_fail;
 ...
 */
		if (win->current_node->annotation_type == mo_annotation_private)
			...
			if (win->current_node->annotation_type == 
				...
				win->pubpri = mo_annotation_workgroup;
		XmxRSetOptionMenuHistory(win->pubpri_menu,(XtPointer)win->pubpri);
